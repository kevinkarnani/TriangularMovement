...
public class Ghost {
	...
	/**
	 * Creates a ghost object for the simulation.
	 * Defines the 3 motors the ghost is attached to.
	 * Instantiates objects used to calculate the ghost position.
	 * Creates a visual representation of the ghost.
	 * 
	 * @param leftMotor   the left motor which the ghost is attached to.
	 * @param topMotor    the top motor which the ghost is attached to.
	 * @param rightMotor  the right motor which the ghost is attached to.
	 */
	...
	/**
	 * Sets the depth limit of the ghost representation
	 * in the simulation.
	 * 
	 * @param depth  the depth limit of the ghost.
	 */
	 ...
	 * @param z  the z coordinate of the ghost.
	 */
	...
	/**
	 * Calculates and stores the next proposed coordinate
	 * of the ghost from a vector.
	 * 
	 * @param vector  the proposed vector which the ghost moves in.
	 */
	 ...
	 * 
	 * <p>Calculates the speed from the distance from a coordinate to one motor and the
	 * distance of the next coordinate to the same motor. Process is repeated for each motor.
	 * 
	 * <p>Amplifies the ratio of the motor speeds to 1, which is the motor max speed.
	 * Sets the motor speeds to the motor.
	 * 
	 * <p>Note: in the Christmas play: the motor ratio will be defined by the distance of
	 * the joystick from the origin of the joystick.
	 *  
	 * @param vector  the desired direction of the ghost.
	 */
	public void calcMotorSpeeds(Vector vector) {
		...
		double topMotorSpeed =  MathUtil.calcSpeed(this.getDistanceFromTop(),
				  				  		 MathUtil.distFromPoints(topMotor.getCoordinate(), this.getNextCoordinate()),
				  				  		 1);
		double leftMotorSpeed = MathUtil.calcSpeed(this.getDistanceFromLeft(),
				  						 MathUtil.distFromPoints(leftMotor.getCoordinate(), this.getNextCoordinate()),
				  						 1);
		double rightMotorSpeed = MathUtil.calcSpeed(this.getDistanceFromRight(),
				  				 		 MathUtil.distFromPoints(rightMotor.getCoordinate(), this.getNextCoordinate()),
				  				 		 1);
	...
	public void calibrate() {
		this.ghostCoord = trilateration.calcPosition(leftMotor.getEncoderValue(), 
